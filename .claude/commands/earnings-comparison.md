...
- **Reverse chronological order**: All tables must show the most recent data first (newest year at top, oldest at bottom). Within each year group, show most recent quarter first.
- **Group unchanged periods**: If guidance was the same for 4 consecutive quarters, show one row: "Jan '23 – Apr '22" instead of 4 identical rows
- **Year divider rows** separate data in reverse chronological order and can have contextual labels (e.g., "2024 — AI ACCELERATION", "2023 — YEAR OF EFFICIENCY")
